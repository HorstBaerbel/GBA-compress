...
There is a GBA example in [gba/gbacomp.cpp](/gba/gbacomp.cpp) that tests and benchmarks decompression on the GBA. It uses 8 XBGR1555 images, which (see table above) seems to be a worst-case scenario for LZ4 ratio wise. gbacomp shrinks these files to **226.3 kB** using LZ4 and 238.7 kB using LZ77, so only a small win for LZ4. LZ4 decompression on GBA is much faster than LZSS. There's an option to use DMA3 to decompress LZ4, which is even faster (but not everybody might like to use DMA for this).

|                                     | LZ4 (DMA) | LZ4             | LZ77^           |
| ----------------------------------- | --------- | --------------- | --------------- |
| Size / kB                           | **226.3** | **226.3**       | 238.7 (+5.5 %)  |
| Decompression / ms (mGBA)           | **251.6** | 312.9 (+24.5 %) | 404.6 (+61.0 %) |
| Decompression / ms (NanoBoyAdvance) | **257.8** | 318.3 (+23.4 %) | 416.7 (+61.6 %) |
