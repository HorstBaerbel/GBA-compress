...
namespace Compression
{
    ...
    auto findBestMatch(const std::vector<uint8_t> &src, const std::multimap<uint32_t, int32_t> &hashPositions, const int32_t srcPosition, const bool vramCompatible) -> MatchInfo
    {
        ...
        const int32_t MaxMatchLength = srcPosition + MAX_MATCH_LENGTH >= src.size() ? src.size() - srcPosition - 1 : MAX_MATCH_LENGTH;
        ...
        while (possibleMatchIt != possibleMatches.second)
        {
            ...
            if (matchStartPosition < srcPosition && distance <= MAX_MATCH_DISTANCE)
            {
                // if we currently have no best match, store this one
                if (bestMatch.distance == 0 && bestMatch.length == 0)
                {
                    bestMatch = {distance, MIN_MATCH_LENGTH};
                }
                // we already know the match length is >= MIN_MATCH_LENGTH, so start with MIN_MATCH_LENGTH + 1
                for (int32_t matchLength = (MIN_MATCH_LENGTH + 1); matchLength <= MaxMatchLength; ++matchLength)
                {
    ...
    auto flushToken(std::vector<uint8_t> &dst, const TokenInfo &token) -> void
    {
        ...
        if (!token.literals.empty())
        {
            ...
            if (storedLiteralLength < 15)
            {
                ...
                tokenByte |= storedLiteralLength << LITERAL_LENGTH_SHIFT;
            }
            else
            {
...
#endif
            // store match offset
            REQUIRE(token.matchOffset > 0 && token.matchOffset <= MAX_MATCH_DISTANCE, std::runtime_error, "Match offset out of range [1,65535]");
            dst.push_back((token.matchOffset >> 8) & 0xFF);
            dst.push_back(token.matchOffset & 0xFF);
            // store literal length
            REQUIRE(token.matchLength >= MIN_MATCH_LENGTH, std::runtime_error, "Match length too small");
            int32_t storedMatchLength = token.matchLength - (MIN_MATCH_LENGTH - 1);
    ...
    auto encodeLZ4_40(const std::vector<uint8_t> &src, bool vramCompatible) -> std::vector<uint8_t>
    {
        ...
        *reinterpret_cast<uint32_t *>(dst.data()) = (src.size() << 8) | TYPE_MARKER;
        // build minmatch hash table for input data. it maps a hash (first MIN_MATCH_LENGTH bytes of data) to its position(s)
        std::multimap<uint32_t, int32_t> hashPositions;
        for (int32_t srcPosition = 0; srcPosition < static_cast<int32_t>(src.size() - MIN_MATCH_LENGTH); ++srcPosition)
        {
...
#pragma omp parallel for
        for (int srcPosition = 0; srcPosition < static_cast<int>(src.size() - MIN_MATCH_LENGTH); ++srcPosition)
        {
...
#pragma omp critical
        ...
        if (!matches.empty())
        {
            ...
            for (int32_t srcPosition = src.size() - 1; srcPosition > matches.cbegin()->first; --srcPosition)
            {
                ...
                if (matchIt != matches.cend())
                {
                    ...
                    auto bestCost = 3 + extraLengthBytesNeeded(currMatchLength - MIN_MATCH_LENGTH) + currentCost;
                    while (currMatchLength >= MIN_MATCH_LENGTH)
                    {
                        ...
                        const auto matchCost = 3 + extraLengthBytesNeeded(currMatchLength - MIN_MATCH_LENGTH) + cost[srcPosition + currMatchLength];
    ...
    auto decodeLZ4_40(const std::vector<uint8_t> &src, bool vramCompatible) -> std::vector<uint8_t>
    {
        ...
        REQUIRE((header & 0xFF) == TYPE_MARKER, std::runtime_error, "Compression type not LZ4 (" << uint32_t(TYPE_MARKER) << ")");
        ...
        while (dst.size() < uncompressedSize)
        {
            // read token
            uint8_t token = *srcIt++;
            uint32_t literalLength = (token >> LITERAL_LENGTH_SHIFT) & LENGTH_MASK;
            uint32_t matchLength = token & LENGTH_MASK;
            if (literalLength > 0)
            {
            ...
            if (matchLength > 0)
            {
                ...
                matchLength += (MIN_MATCH_LENGTH - 1);
#ifdef DEBUG_TOKENS
